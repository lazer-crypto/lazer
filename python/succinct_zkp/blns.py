...
# def description_transform_to_Falcon_modulo(dimension_vector_modq): # each polynomial of the output is decomposed in 2 parts to support the proof
#     dimension_vector_modq_decomposed = dimension_vector_modq * 2
#     base_decomp_Falcon = ceil(sqrt(1.85 * RING_FALCON.mod / sqrt(12)))
#     return dimension_vector_modq_decomposed, base_decomp_Falcon

# def transform_to_Falcon_modulo(binary_vector, matrix_modulus_conversion, dimension_vector_modq, dimension_vector_modq_decomposed):
...
#         decomposed_component = opt_decompose(center_reduce_polynomial(tmp.copy(), RING_FALCON.mod), base_decomp_Falcon, 2) # use opt_decompose instead
#         for j in range(2):
...
# def statements_transform_to_Falcon_modulo(indices_binary_vector, dimension_vector_modq, base_decomp_Falcon, matrix_modulus_conversion, number_witnesses, list_degrees, list_number_polynomials, list_norm_constraints, number_constraints):
#     list_degrees += [RING_PROOF.deg] * (3 * dimension_vector_modq)
#     list_number_polynomials += [1] * (3 * dimension_vector_modq)
#     sqnorm_components_t = RING_PROOF.deg * base_decomp_Falcon ** 2 // 2 # recompute
#     sqnorm_lifting = round(base_decomp_Falcon ** 4 * RING_PROOF.deg * (sqrt(matrix_modulus_conversion.dim * RING_PROOF.deg / 2) + 1) ** 2 / RING_FALCON.mod ** 2)
#     list_norm_constraints += [sqnorm_components_t, sqnorm_components_t, sqnorm_lifting] * dimension_vector_modq
...
#     for i in range(dimension_vector_modq):
#         indices_witness = [indices_binary_vector[i * matrix_modulus_conversion.dim + j] for j in range(matrix_modulus_conversion.dim)] + [number_witnesses + j for j in range(3)]
#         indices_vector_modq += [number_witnesses + j for j in range(2)]
#         number_witnesses += 3
#         statements_modulus_transf.append([left_list, indices_witness, right_pol])
#         number_constraints += len(statements_modulus_transf)
#     return indices_vector_modq, statements_modulus_transf, number_witnesses, list_degrees, list_number_polynomials, list_norm_constraints, number_constraints

# def proof_transform_to_Falcon_modulo(PS, trace_modulus_transf, statements_modulus_transf):
...
def transform_to_Falcon_modulo(binary_vector, matrix_modulus_conversion, dimension_vector_modq, PS, indices_binary_vector, next_witness):
    base_decomp_Falcon = ceil(sqrt(1.85 * RING_FALCON.mod / sqrt(12)))
    dimension_vector_modq_decomposed = dimension_vector_modq * 2
    vector_modq_decomposed = polyvec_t(RING_PROOF, dimension_vector_modq_decomposed)
    trace_modulus_transf = []
    ...
    for statement in statements_modulus_transf:
        PS.append_statement(statement[0], statement[1], statement[2])
    return vector_modq_decomposed, PS, indices_vector_modq, next_witness
...
# def statements_binary_randomness(indices_randomness, dimension_randomness, bound_rand, bits_coefficients_rand, number_witnesses, list_degrees, list_number_polynomials, list_norm_constraints, number_constraints):
#     number_witnesses_bin_decomp_rand = (bits_coefficients_rand + 1) * dimension_randomness
#     list_degrees += [RING_PROOF.deg] * number_witnesses_bin_decomp_rand
#     list_number_polynomials += [1] * number_witnesses_bin_decomp_rand
#     sqnorm_lifting_bin_rand = RING_PROOF.deg # to recompute
#     list_norm_constraints += ([NORM_BINARY] * bits_coefficients_rand + [sqnorm_lifting_bin_rand]) * dimension_randomness
#     indices_binary_randomness = [number_witnesses + (bits_coefficients_rand + 1) * i + j for i in range(dimension_randomness) for j in range(bits_coefficients_rand)]
...
#         ind = number_witnesses + (bits_coefficients_rand + 1) * i
...
#     number_witnesses += number_witnesses_bin_decomp_rand
#     number_constraints += len(statements_binary_decomposition_randomness)
#     return indices_binary_randomness, statements_binary_decomposition_randomness, number_witnesses, list_degrees, list_number_polynomials, list_norm_constraints, number_constraints

# def proof_binary_decomposition_randomness(PS, trace_binary_decomp_rand, statements_binary_decomposition_randomness):
...
def description_compute_binary_randomness(dimension_randomness, bits_coefficients_rand, PS_description):
    number_witnesses_bin_decomp_rand = (bits_coefficients_rand + 1) * dimension_randomness
...
def compute_binary_randomness(randomness, dimension_binary_randomness, bound_rand, bits_coefficients_rand, PS, indices_randomness, next_witness):
    binary_randomness = polyvec_t(RING_PROOF, dimension_binary_randomness)
    trace_binary_decomp_rand = []
    for i in range(randomness.dim):
        decomposed_component = polynomial_binary_decomposition(reduce_polynomial(randomness.get_elem(i).copy(), 2 * bound_rand + 1), 2 ** bits_coefficients_rand) # directly how many bits for decompositions instead of modulus
    ...
    for statement in statements_binary_decomposition_randomness:
        PS.append_statement(statement[0], statement[1], statement[2])
    return binary_randomness, PS, indices_binary_randomness, next_witness
...
if __name__ == "__main__":
    ...
    # description message, nu (for now nu is a binary vector of polynomials)
    dimension_msg = 1
    # update description witnesses for message
    PS_description.list_degrees += [RING_PROOF.deg] * dimension_msg
    PS_description.list_number_polynomials += [1] * dimension_msg
    PS_description.list_norm_constraints += [NORM_BINARY] * dimension_msg
    ...
    PS_description = description_compute_binary_randomness(dimension_randomness, bits_coefficients_rand, PS_description)

    # description hash(bin(r))
    dimension_binary_hashed_randomness = 1
    PS_description = description_hash(dimension_binary_randomness, dimension_binary_hashed_randomness, PS_description)

    # description binary input hash, rho = hash(bin(r)) || nu
    dimension_input_hash_rho = dimension_binary_hashed_randomness + dimension_msg
    ...
    PS_description = description_hash(dimension_input_hash_rho, dimension_binary_hashed_rho, PS_description)
    ...
    sqnorm_lifting_commitment = round((1.45 * (sqrt(RING_PROOF.deg) + 1)) ** 2 * RING_PROOF.deg / 3)
    PS_description.list_norm_constraints += [sqnorm_lifting_commitment]
    PS_description.number_constraints += 1

    ## Execution
    # proof setup
    PS_commitment = proof_statement(PS_description.list_degrees, PS_description.list_number_polynomials, PS_description.list_norm_constraints, PS_description.number_constraints, SIZE_MODULUS_PROOF, zk = True)
    next_witness = 0
    ...
    for i in range(dimension_msg):
        PS_commitment.append_witness(message.get_elem(i))
    indices_msg = [next_witness + i for i in range(dimension_msg)]
    next_witness += dimension_msg
    ...
    sqnorm_randomness = 0
    for i in range(dimension_randomness):
        sqnorm_randomness += randomness.get_elem(i).l2sq()
    ...
    indices_randomness = [next_witness + i for i in range(dimension_randomness)]
    next_witness += dimension_randomness

    # binary randomness
    binary_randomness, PS_commitment, indices_binary_randomness, next_witness = compute_binary_randomness(randomness, dimension_binary_randomness, bound_rand, bits_coefficients_rand, PS_commitment, indices_randomness, next_witness)

    # binary input hash, rho = hash(r) || nu
    initial_vector_rand = polyvec_t(RING_PROOF, BITS_MODULUS_HASH)
    hash1_start_time = time.perf_counter()
    binary_hashed_randomness, PS_commitment, indices_binary_hashed_rand, next_witness = hash(binary_randomness, initial_vector_rand, dimension_binary_hashed_randomness, PS_commitment, indices_binary_randomness, next_witness)
    hash1_end_time = time.perf_counter()
    ...
    hash2_start_time = time.perf_counter()
    binary_hashed_rho, PS_commitment, indices_binary_hashed_rho, next_witness = hash(input_hash_rho, initial_vector_rho, dimension_binary_hashed_rho, PS_commitment, indices_input_hash_rho, next_witness)
    hash2_end_time = time.perf_counter()

    # hash(rho) mod q, h = h(0) + b * h(1)
    # add witnesses and statements from modulus transformation of hashed_rho
    hashed_rho_modq_decomposed, PS_commitment, indices_hashed_rho_modq, next_witness = transform_to_Falcon_modulo(binary_hashed_rho, matrix_modulus_conversion, dimension_hashed_rho_modq, PS_commitment, indices_binary_hashed_rho, next_witness)
    ...
    print("Times Commitment")
    print("Time hash 1: ", hash1_end_time - hash1_start_time)
    print("Time hash 2:", hash2_end_time - hash2_start_time)
    print("Time proof:", proof_end_time - proof_start_time)
    print("Time verification:", verification_end_time - verification_start_time, "\n")
